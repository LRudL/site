...
def create_post_groupings(posts: list[Post]) -> PostGroupings:
    ...
    for post_type, post_type_posts in groupby(
        sorted(posts, key=lambda x: x.meta.type), key=lambda x: x.meta.type
    ):
        posts_by_type.append(
            {
                "type": post_type_list_to_str(post_type),
                "posts": sorted(
                    list(post_type_posts), key=lambda x: x.meta.date, reverse=True
                ),
            }
        )
